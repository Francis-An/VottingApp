      ...
      child: Drawer(
        ...
        child: ListView(padding: EdgeInsets.all(0.0), children: <Widget>[
          UserAccountsDrawerHeader(
            accountName: Obx(
              () => Text(Get.find<UserController>().user.name.toString()),
              // () => Text(""),
            ),
            accountEmail: Obx(
              // () => Text(""),
              () => Text(Get.find<UserController>().user.email.toString(),
                  style: TextStyle(color: Colors.white54)),
            ),
            currentAccountPicture: CircleAvatar(
                radius: 60.0,
                backgroundImage:
                    NetworkImage(Get.find<UserController>().user.avatar.toString())
                    ),
